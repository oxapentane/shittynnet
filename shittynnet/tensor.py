...
class Tensor:
  ...
  def exp(self):
    out = Tensor(np.nan_to_num(np.exp(self.data)), (self,), 'exp')
    def _backward():
      self.grad += np.nan_to_num(np.exp(self.data)) * out.grad
    out._backward = _backward
    return out
  ...
  def mean(self):
    ...
    def _backward():
      self.grad += self._val_shape(out.grad) / self.data.size
    out._backward = _backward
    return out
  ...
  def sigm(self): return ((-self).exp() + self._val_shape(1))**-1
  ...
  def __neg__(self): return self * self._val_shape(-1)
